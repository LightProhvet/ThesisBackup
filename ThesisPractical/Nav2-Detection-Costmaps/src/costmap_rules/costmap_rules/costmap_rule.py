...
# ros 2 imports
import rclpy
from std_msgs import Header  # this is imported differently in ros1?

from rclpy.node import Node
from nav_msgs.msg import OccupancyGrid, MapMetaData
from nav2_msgs.msg import Costmap, CostmapMetaData  # ros2/Nav2 eqvuivalent to OccupancyGrid and MapMetaData
...
class CostmapRule(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                # detection params (Used also by the hungarian node,
                # advisable to remap these into whatever they are remapped to in the kf_hungarian_node)
                ('costmap_height_filter', [-2.0, 2.0]),
        ...
        # Save params. You can find explanations for each here as well:
        ...
        # The of the robot. Obstacles outside of this range are rendered obsolete (not as obstacles)
        self.costmap_height_filter = self.get_parameter("costmap_height_filter")._value
    ...
    def costmap_include_2d(self, height, inclusive=True):
        if inclusive:
            return self.costmap_height_filter[0] <= height <= self.costmap_height_filter[1]
        else:
            return self.costmap_height_filter[0] < height < self.costmap_height_filter[1]

    def get_costmap_dictionary(self, costmap_3d_list, filter_height=True):
        # Initialize a dictionary to aggregate the data
        costmap_3d_dict = {}
        # Aggregate the data
        for cost, certainty, coordinates in costmap_3d_list:
            x, y, z = coordinates
            if not self.costmap_include_2d(z) and filter_height:
                continue
            if (x, y) in costmap_3d_dict:
                costmap_3d_dict[(x, y)].append((cost, certainty))
            else:
                costmap_3d_dict[(x, y)] = [(cost, certainty)]
        return costmap_3d_dict
    ...
    def remove_dimension(self, costmap_3d, dimension="z"):
        """
        Removes a dimension (z by default) for a 3D costmap.
        Assumes the 3D costmap has been generated using Points (x,y, z) where z is height
        :param costmap_3d: list of with data: [[cost, probability, coordinates([x,y,z])]
        :param dimension: the dimension to be removed. See 'index' variable
        :return: costs in
        """
        # TODO?format: might change, see "5) apply costs"
        # TODO?performance: do we want to allow removing other dimensions?
        index = {'x': 0, 'y': 0, 'z': 2}
        d = index.get(dimension, 2)
        return [[cost[0], cost[1]] if self.costmap_include_2d(cost[2][d]) else [0, 0] for cost in costmap_3d]

    def format_costmap(self, detections, nav1=False):
        """
        Function to format detections suitable for costmap. This is used after calculating costs and
        considers robot height (costmap_height_filter) to only account for detections that are relevant.
        We use it after cost calculations to allow capture of movements from outside the filter towards the filter
        :param detections (DetectionCostsArray):
        :return: Costmap
        """
        # TODO: are the 3d detections already in
        costmap_2d = self.remove_dimension(detections)
        if nav1:
            # Occupancy Grid instead of Costmap
            costmap_detections = OccupancyGrid()
            # where
            # Header instead of std_msgs/Header (?)
            costmap_detections.Header = False  # TODO:
        else:
            costmap_detections = Costmap()
            # CostmapMetaData instead of MapMetaData

        return costmap_detections
    ...
    def get_costmap(self, msg="Not needed?"):
        ...
        # 5) apply costs TODO: apply rules unto 3D costmap
        # 6) format output
        output = self.map_detections_to_costmap(costmap_3d)
