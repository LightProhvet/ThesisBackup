...
# Updated version of the code from here https://github.com/SeungBack/open3d-ros-helper
...
def pq_to_pose_stamped(p, q, source_frame, target_frame, current_time, stamp=None):
    ...
    if stamp is None: stamp = current_time
...
def pq_to_transform_stamped(p, q, source_frame, target_frame, current_time, stamp=None):
...
def se3_to_transform_stamped(transform_nparray, source_frame, target_frame, current_time, stamp=None):
...
def rospc_to_o3dpc(rospc, remove_nans=False):
    ...
    cloud_array = point_cloud2_to_array(rospc)
    if remove_nans:
        mask = np.isfinite(cloud_array['x']) & np.isfinite(cloud_array['y']) & np.isfinite(cloud_array['z'])
        cloud_array = cloud_array[mask]
    if is_rgb:
        cloud_npy = np.zeros(cloud_array['xyz'].shape + (4,), dtype=np.float64)
    else: 
        cloud_npy = np.zeros(cloud_array['xyz'].shape + (3,), dtype=np.float64)
    
    cloud_npy[...,0] = cloud_array['xyz'][0]
    cloud_npy[...,1] = cloud_array['xyz'][1]
    cloud_npy[...,2] = cloud_array['xyz'][2]
...
def o3dpc_to_rospc(o3dpc, current_time, frame_id=None, stamp=None):
    ...
    if stamp is None:
        rospc.header.stamp = current_time
    else:
